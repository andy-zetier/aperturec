...
impl Distribution<DamageShape> for StandardUniform {
    ...
    (0..num_areas)
        .map(|_| {
            let origin = Point::new(
                rng.random_range(0..total_area.width - MIN_LEN),
                rng.random_range(0..total_area.height - MIN_LEN),
            );
            let max_width = total_area.width - origin.x;
            let max_height = total_area.height - origin.y;
            let size = Size::new(
                rng.random_range(MIN_LEN..=max_width),
                rng.random_range(MIN_LEN..=max_height),
            );
            ...
            let damaged_width = rng.random_range(0..=area.width());
            let damaged_height = rng.random_range(0..=area.height());
            let damage_shape = rand::random::<DamageShape>();
