...
/// * `MyMetric::update_last(FnOnce(f64))` Provides the last value to a closure and stores the result
/// * `MyMetric::update(new_value)` Sets the metric value to `new_value`
/// * `MyMetric::inc()` Increments the metric by 1
/// * `MyMetric::dec()` Decrements the metric by 1
/// * `MyMetric::inc_by(count)` Increments the metric by `count`
/// * `MyMetric::dec_by(count)` Decrements the metric by `count`
/// * `MyMetric::register()` Registers the Metric. Must be called prior to use.
/// * `MyMetric::register_sticky()` Registers the Metric. This version of a Stats Metric will retain the last value across `poll()` calls instead of being reset to 0.
///
...
///
/// * `MyHistogram::observe(value)` Calls [`observe()`](prometheus::Histogram::observe) on the generated histogram metric
///
