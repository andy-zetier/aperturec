...
use crossbeam_ring_channel::{RingReceiver, RingSender, Select, SelectedOperation, ring_bounded};
...
impl CancellationToken {
    ...
    ///
    /// Cancellation is one-shot; create a new token to reuse.
    pub fn cancel(&self) {
...
pub enum DisplayMode {
    ...
    Windowed { size: Size },
    /// Single-monitor fullscreen using the first provided display.
    SingleFullscreen,
...
impl Drop for Connection {
    fn drop(&mut self) {
            ...
            && let Err(panic) = handle.join()
        {
            ...
            error!(%panic_msg, "connection primary thread panicked during shutdown");
        }
    }
}
...
fn handle_pending_quit(
    pending_quit: &mut Option<(Instant, QuitReason)>,
    force_timeout: bool,
    from_cc_rx: &Receiver<FromCC>,
    handle_cc_msg: &impl Fn(FromCC) -> bool,
    shutdown_initiated_pt: &Arc<AtomicBool>,
    event_lossless_tx: &Sender<Event>,
) -> PendingQuitOutcome {
    let Some((deadline, _)) = pending_quit else {
        return PendingQuitOutcome::None;
    };
    if !force_timeout && Instant::now() < *deadline {
        return PendingQuitOutcome::None;
    }
    if let Ok(cc_msg) = from_cc_rx.try_recv() {
        if handle_cc_msg(cc_msg) {
            return PendingQuitOutcome::Break;
        }
        return PendingQuitOutcome::Continue;
    }
...
impl Connection {
    ...
    /// Returns `true` while the connection is fully active.
    ///
    /// A connection is considered active as long as the primary connection thread is running and
    /// it is still possible to receive events or send input. Once the connection has fully closed
    /// and the primary thread has exited, this returns `false` permanently.
    ...
    ///
    /// This flag becomes `true` when a disconnect is requested or an internal shutdown path begins,
    /// but before the connection has fully closed. During this period `is_active()` may still be
    /// `true` until all in-flight work is drained and the primary thread exits.
...
impl Client {
    fn get_init(&self) -> Result<&proto::control::ClientInit, ConnectionError> {
        if let Some(ci) = self.client_init.get() {
            return Ok(ci);
        }

        let client_info = self.get_info()?.clone();
        ...
        let client_init = {
            ...
            let client_init = proto::control::ClientInitBuilder::default()
                .auth_token(config.auth_token.expose_secret())
                .client_info(client_info)
                .client_caps(client_caps)
                .max_decoder_count(config.decoder_max.get() as u32)
                .client_specified_program_cmdline(
                    config.program_cmdline.clone().unwrap_or_default(),
                )
                ...
                .build()
                .map_err(|error| {
                    ConnectionError::InvalidClientConfiguration(format!(
                        "failed to generate ClientInit: {error}"
                    ))
                })?;
            config.auth_token.zeroize();
            client_init
        };

        let _ = self.client_init.set(client_init);
        Ok(self
            .client_init
            .get()
            .expect("client init should be available after set"))
    }

    fn get_info(&self) -> Result<&proto::control::ClientInfo, ConnectionError> {
        if let Some(info) = self.client_info.get() {
            return Ok(info);
        }
        ...
        let displays = match self.config.borrow().initial_display_mode {
            DisplayMode::Windowed { size } => vec![proto::common::DisplayInfo {
                area: Some(
            ...
            DisplayMode::SingleFullscreen => {
                let display = self.initial_displays_request.first().ok_or_else(|| {
                    ConnectionError::InvalidClientConfiguration(
                        "no displays available for SingleFullscreen".to_string(),
                    )
                })?;
        ...
        let os = match consts::OS {
            ...
            other => {
                return Err(ConnectionError::UnsupportedPlatform(format!("os: {other}")));
            }
        };
        ...
        let client_info = proto::control::ClientInfoBuilder::default()
            ...
            .map_err(|error| {
                ConnectionError::InvalidClientConfiguration(format!(
                    "failed to generate ClientInfo: {error}"
                ))
            })?;

        let _ = self.client_info.set(client_info);
        Ok(self
            .client_info
            .get()
            .expect("client info should be available after set"))
    }

    /// Validates server tunnel responses against the requested tunnel parameters.
    fn validate_tunnel_responses(
        &self,
        tunnel_responses: &BTreeMap<u64, proto::tunnel::Response>,
    ) -> Result<(), ConnectionError> {
        for tunnel_id in self.requested_tunnels.keys() {
            if !tunnel_responses.contains_key(tunnel_id) {
                return Err(ConnectionError::UnexpectedServerBehavior(format!(
                    "missing tunnel response for request {tunnel_id}"
                )));
            }
        }

        for (tunnel_id, response) in tunnel_responses {
            let Some(expected) = self.requested_tunnels.get(tunnel_id) else {
                return Err(ConnectionError::UnexpectedServerBehavior(format!(
                    "received tunnel response for unknown request {tunnel_id}"
                )));
            };
            let Some(message) = response.message.as_ref() else {
                return Err(ConnectionError::UnexpectedServerBehavior(format!(
                    "tunnel response {tunnel_id} missing message"
                )));
            };

            let proto::tunnel::response::Message::Success(actual) = message else {
                continue;
            };

            let expected_side = proto::tunnel::Side::try_from(expected.side).map_err(|_| {
                ConnectionError::UnexpectedServerBehavior(format!(
                    "tunnel request {tunnel_id} has invalid side {}",
                    expected.side
                ))
            })?;
            let actual_side = proto::tunnel::Side::try_from(actual.side).map_err(|_| {
                ConnectionError::UnexpectedServerBehavior(format!(
                    "tunnel response {tunnel_id} has invalid side {}",
                    actual.side
                ))
            })?;
            if expected_side != actual_side {
                return Err(ConnectionError::UnexpectedServerBehavior(format!(
                    "tunnel response {tunnel_id} side mismatch (expected {expected_side:?}, got {actual_side:?})"
                )));
            }

            let expected_protocol =
                proto::tunnel::Protocol::try_from(expected.protocol).map_err(|_| {
                    ConnectionError::UnexpectedServerBehavior(format!(
                        "tunnel request {tunnel_id} has invalid protocol {}",
                        expected.protocol
                    ))
                })?;
            let actual_protocol =
                proto::tunnel::Protocol::try_from(actual.protocol).map_err(|_| {
                    ConnectionError::UnexpectedServerBehavior(format!(
                        "tunnel response {tunnel_id} has invalid protocol {}",
                        actual.protocol
                    ))
                })?;
            if expected_protocol != actual_protocol {
                return Err(ConnectionError::UnexpectedServerBehavior(format!(
                    "tunnel response {tunnel_id} protocol mismatch (expected {expected_protocol:?}, got {actual_protocol:?})"
                )));
            }

            if expected.forward_address != actual.forward_address
                || expected.forward_port != actual.forward_port
            {
                return Err(ConnectionError::UnexpectedServerBehavior(format!(
                    "tunnel response {tunnel_id} forward mismatch (expected {}:{}, got {}:{})",
                    expected.forward_address,
                    expected.forward_port,
                    actual.forward_address,
                    actual.forward_port
                )));
            }

            // Server-bound tunnels may return a concrete bind port/address when the request
            // specified port 0 or left the bind address empty.
            if if expected_side == proto::tunnel::Side::Client {
                expected.bind_address != actual.bind_address
                    || expected.bind_port != actual.bind_port
            } else {
                (!expected.bind_address.is_empty() && expected.bind_address != actual.bind_address)
                    || (expected.bind_port != 0 && expected.bind_port != actual.bind_port)
            } {
                return Err(ConnectionError::UnexpectedServerBehavior(format!(
                    "tunnel response {tunnel_id} bind mismatch (expected {}:{}, got {}:{})",
    ...
    ///
    /// For [`DisplayMode::SingleFullscreen`], at least one display must be provided.
    /// For [`DisplayMode::MultiFullscreen`], only enabled displays are requested.
    pub fn new<'a>(
    ...
    ///
    /// The client validates server tunnel allocations against the requested tunnel parameters.
    pub fn connect(&mut self) -> Result<Connection, ConnectionError> {
        ...
        self.validate_tunnel_responses(&server_init.tunnel_responses)?;
        ...
        let primary_thread = thread::spawn(move || {
            ...
            let handle_pt_msg = |pt_msg: Notification,
                                 to_cc_tx: &Sender<ToCC>,
                                 to_ec_lossless_tx: &Sender<ToEC>,
                                 to_ec_lossy_tx: &RingSender<ec::UserEvent>,
                                 to_mc_tx: &Sender<ToMC>,
                                 to_tc_tx: &Sender<ToTC>,
                                 shutdown_initiated_pt: &Arc<AtomicBool>|
             -> bool {
                match pt_msg {
            ...
            loop {
                ...
                match handle_pending_quit(
                    &mut pending_quit,
                    false,
                    &from_cc_rx,
                    &handle_cc_msg,
                    &shutdown_initiated_pt,
                    &event_lossless_tx,
                ) {
                    PendingQuitOutcome::Break => break,
                    PendingQuitOutcome::Continue => continue,
                    PendingQuitOutcome::None => {}
                }

                match pt_rx.try_recv() {
                    Ok(pt_msg) => {
                        if handle_pt_msg(
                            pt_msg,
                            &to_cc_tx,
